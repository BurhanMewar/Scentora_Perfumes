      ...
      <section className="py-10 md:py-14">
        <div className="grid gap-10 lg:grid-cols-12 lg:items-end">
          <div className="lg:col-span-7">
            ...
            <h1 className="mt-3 font-heading text-3xl font-semibold leading-tight text-textPrimary sm:text-4xl md:text-5xl">
          ...
          <div className="lg:col-span-5">
            <div className="rounded-lg border border-black/10 bg-white/80 p-6 shadow-card">
              ...
              <div className="mt-5 space-y-4 text-sm text-textSecondary">
                {settings.contactPhone ? <ContactLink href={`tel:${settings.contactPhone}`} icon={<Phone className="h-5 w-5" aria-hidden="true" />} label="Phone" value={settings.contactPhone} /> : null}
                {settings.contactEmail ? <ContactLink href={`mailto:${settings.contactEmail}`} icon={<Mail className="h-5 w-5" aria-hidden="true" />} label="Email" value={settings.contactEmail} /> : null}
                {settings.contactPhone ? <ContactLink href={`https://wa.me/${settings.contactPhone.replace(/\D/g, "")}`} icon={<MessageCircle className="h-5 w-5" aria-hidden="true" />} label="WhatsApp" value="Message our team" /> : null}
              </div>
            </div>
          </div>
        </div>
